    ...
    <div class="joke">
        <div class="section">
            ...
            <div v-if="jokes.length > 0">
                <div v-for="joke in jokes" :key="joke.id">{{ joke.joke }} (id: {{ joke.id }})</div>
            </div>
            <div class="btn-group">
                <button @click="getAJoke" class="btn btn-primary">Get a joke</button>
                <button @click="getMultiJoke(3)" class="btn btn-primary">Get multi joke</button>
            </div>
        </div>
        <div class="section">
            <p>Search joke:</p>
            <input type="text" v-model="$v.search_joke.$model" />
            <div class="error" v-if="$v.search_joke.$error && !$v.search_joke.required">Input is required!</div>
            <div class="result" v-if="jokeFound.status === 200">{{ jokeFound.joke }} ({{ jokeFound.id }})</div>
            <div class="result" v-else>{{ jokeFound.message }}</div>
            <div class="result" v-if="searched && totalJokesFound > 0">Found {{ totalJokesFound }} jokes</div>
            <div class="result" v-if="searched && totalJokesFound === 0">No joke found</div>

            <div class="btn-group">
                <button @click="searchJokeByID" class="btn btn-primary">Search by ID</button>
                <button @click="searchJokeByWords" class="btn btn-primary">Search Joke by Words</button>
            </div>
        </div>
        <div>
            <p>Async Validation with Vuelidate (Validate id)</p>
            <input type="text" v-model="$v.asyncValidate.$model" />
            <div v-if="is_loading">Checking...</div>
            <div v-else>
                <div class="error" v-if="$v.asyncValidate.$error && !$v.asyncValidate.required">Id is required!</div>
                <div class="success" v-if="$v.asyncValidate.required && $v.asyncValidate.isUnique">
                    {{ asyncValidate }} is an valid joke id
                </div>
                <div class="error" v-if="$v.asyncValidate.required && !$v.asyncValidate.isUnique">
